...
#----------------------------------------------------------------
station = st.sidebar.selectbox(label="Choose a train station", options=train_stations.index, label_visibility="visible")
...
if len(source) == 0:
    st.sidebar.warning('Try a larger buffer', icon="📏")
